...
- **Applicability** — Whether an action applies to a project. The tool that
  an action wraps decides it: a run that the tool reports had nothing to
  examine skips with a visible message. An action examines nothing of its own
  beforehand, so one discovery answers both what a run examined and what it
  found.
