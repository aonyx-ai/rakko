...
impl Project {
    ...
    async fn run(&self, fix: bool) -> Outcome {
        ...
        let args = FormatMarkdownArgs::from_values(&values)
            .expect("the test builds the arguments from values that the action reads");

        FormatMarkdown.run(&self.context(), &args).await
    }
...
// formatmarkdown[verify name]
#[test]
fn action_identifies_itself_as_format_markdown() {
    let name = FormatMarkdown.name();

    assert_eq!(name.get(), "format-markdown");
}

// formatmarkdown[verify tool.prettier]
// formatmarkdown[verify check.passed]
#[tokio::test]
...
// formatmarkdown[verify skip.missing]
#[tokio::test]
async fn run_in_a_project_without_markdown_names_what_it_looked_for() {
...
// formatmarkdown[verify skip.missing]
#[tokio::test]
async fn run_in_a_project_without_markdown_skips() {
    let project = Project::bare();
    project.write("notes.txt", FORMATTED);
...
// formatmarkdown[verify fix.partial]
#[cfg(unix)]
#[tokio::test]
async fn run_with_an_unreadable_file_and_fix_keeps_the_problem() {
    ...
    project.write("secret.md", FORMATTED);
...
// formatmarkdown[verify check.unreadable]
#[cfg(unix)]
#[tokio::test]
...
async fn run_with_an_unreadable_file_names_the_file() {
    ...
    assert_eq!(
        findings[0].location(),
        &Location::File {
            path: "secret.md".parse().expect("the test names a relative path"),
        }
    );
}

// formatmarkdown[verify check.unformatted]
#[tokio::test]
async fn run_with_an_unformatted_file_names_the_file() {
    let project = Project::new();
    project.write("sub/messy.md", UNFORMATTED);
...
// formatmarkdown[verify fix.changed]
#[tokio::test]
...
// formatmarkdown[verify fix.write]
#[tokio::test]
async fn run_with_fix_rewrites_the_file() {
    ...
    assert_eq!(project.read("messy.md"), FORMATTED);
}

// formatmarkdown[verify skip.dependencies]
#[tokio::test]
async fn run_with_markdown_only_under_the_dependencies_skips() {
    let project = Project::bare();
    project.write("node_modules/package/README.md", UNFORMATTED);
...
// formatmarkdown[verify skip.git]
#[tokio::test]
async fn run_with_markdown_only_under_the_git_directory_skips() {
    let project = Project::bare();
    project.write(".git/description.md", UNFORMATTED);
...
// formatmarkdown[verify skip.links]
#[cfg(unix)]
#[tokio::test]
async fn run_with_markdown_only_behind_a_symbolic_link_skips() {
    ...
    std::fs::write(elsewhere.path().join("linked.md"), UNFORMATTED)
...
// formatmarkdown[verify check.read]
#[tokio::test]
async fn run_without_fix_leaves_the_file_unchanged() {
    ...
    assert_eq!(project.read("messy.md"), UNFORMATTED);
}

// formatmarkdown[verify tool.missing]
#[tokio::test]
