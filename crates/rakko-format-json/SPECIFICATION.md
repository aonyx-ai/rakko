...
`rakko-format-json` provides the action that formats the JSON files of
...
JSON, JSON, and YAML in one call, and a check that fails over all of them
...
The action applies to a project that holds JSON files. The examination is
a cheap look of the action's own, and it runs before the tool resolves, so
that a broad bundle stays safe: a project without JSON files and without a
...
matches the `.json` and `.json5` extensions with the case that prettier
...
formatjson[skip.missing]
A run in a project that holds no file with the `.json` or the `.json5`
...
JSON files that its ignore files leave and names what it cannot leave
