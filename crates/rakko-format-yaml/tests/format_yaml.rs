...
use rakko_format_yaml::{FormatYaml, FormatYamlArgs};
use tempfile::TempDir;

/// A YAML document that prettier leaves alone
const FORMATTED: &str = "a: 1\n";

/// A YAML document that prettier rewrites
const UNFORMATTED: &str = "a:    1\n";

/// A YAML document that prettier cannot parse
const INVALID: &str = "a: [1, 2\n";
...
impl Project {
    ...
    async fn run(&self, fix: bool) -> Outcome {
        ...
        let args = FormatYamlArgs::from_values(&values)
            .expect("the test builds the arguments from values that the action reads");

        FormatYaml.run(&self.context(), &args).await
    }
...
// formatyaml[verify name]
#[test]
fn action_identifies_itself_as_format_yaml() {
    let name = FormatYaml.name();

    assert_eq!(name.get(), "format-yaml");
}

// formatyaml[verify tool.prettier]
// formatyaml[verify check.passed]
#[tokio::test]
async fn run_in_a_formatted_project_passes() {
    let project = Project::new();
    project.write("clean.yaml", FORMATTED);
...
// formatyaml[verify skip.missing]
#[tokio::test]
async fn run_in_a_project_without_yaml_names_what_it_looked_for() {
...
// formatyaml[verify skip.missing]
#[tokio::test]
async fn run_in_a_project_without_yaml_skips() {
    let project = Project::bare();
    project.write("notes.txt", FORMATTED);
...
// formatyaml[verify check.configuration]
#[tokio::test]
...
async fn run_with_an_unreadable_file_and_fix_keeps_the_problem() {
    ...
    let project = Project::new();
    project.write("messy.yaml", UNFORMATTED);
    project.write("secret.yaml", FORMATTED);
...
// formatyaml[verify check.unreadable]
#[cfg(unix)]
#[tokio::test]
...
async fn run_with_an_invalid_file_carries_the_message_of_prettier() {
    let project = Project::new();
    project.write("broken.yaml", INVALID);
...
async fn run_with_an_invalid_file_reports_the_position() {
    ...
    assert_eq!(
        ...
        (
            "broken.yaml".to_owned(),
            Position::builder().line(2).column(1).build()
        )
    );
}

// formatyaml[verify check.unformatted]
#[tokio::test]
async fn run_with_an_unformatted_file_names_the_file() {
    let project = Project::new();
    project.write("sub/messy.yaml", UNFORMATTED);
...
async fn run_with_fix_rewrites_the_file() {
    ...
    assert_eq!(project.read("messy.yaml"), FORMATTED);
}

// formatyaml[verify skip.dependencies]
#[tokio::test]
async fn run_with_yaml_only_under_the_dependencies_skips() {
    let project = Project::bare();
    project.write("node_modules/package/README.yaml", UNFORMATTED);
...
// formatyaml[verify skip.git]
#[tokio::test]
async fn run_with_yaml_only_under_the_git_directory_skips() {
    let project = Project::bare();
    project.write(".git/description.yaml", UNFORMATTED);
...
async fn run_with_yaml_only_behind_a_symbolic_link_skips() {
    ...
    std::fs::write(elsewhere.path().join("linked.yaml"), UNFORMATTED)
...
async fn run_without_fix_leaves_the_file_unchanged() {
    ...
    assert_eq!(project.read("messy.yaml"), UNFORMATTED);
}

// formatyaml[verify tool.missing]
#[tokio::test]
