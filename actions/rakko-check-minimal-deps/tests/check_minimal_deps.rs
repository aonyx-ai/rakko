...
//! on what crates.io published this morning. A floor that no registry serves
//! resolves to itself, so a test reads whether the run resolved and tested at
//! all, and cargo answers for which versions it picks.
//!
...
/// A library with one test that fails when the nightly built it
///
/// The proxy of rustup names the toolchain of a build in the environment, and
/// the compiler hands the value to the code that it builds. A build that the
/// nightly of the update produced therefore fails the test, and a build on the
/// toolchain that the project pins passes it.
const REPORTING_TOOLCHAIN: &str = "#[cfg(test)]\nmod tests {\n    #[test]\n    fn builds_on_the_toolchain_of_the_project() {\n        let toolchain = option_env!(\"RUSTUP_TOOLCHAIN\").unwrap_or_default();\n\n        assert!(!toolchain.starts_with(\"nightly\"), \"built on {toolchain}\");\n    }\n}\n";
...
impl Project {
    ...
    /// the nextest, and the nightly toolchain that mise resolves for it are
    /// the ones that the repository pins and installs. Mise ignores a
    ...
    /// Creates a project that pins a Rust toolchain but no nightly channel
    ///
    /// The pin names the toolchain that this repository builds with, so mise
    /// reports a cargo and the run reaches the toolchain of the update. A pin
    /// of the project replaces the pin of the machine for the same tool, so
    /// the global configuration adds no nightly to it.
    async fn without_nightly() -> Self {
        let project = Self::bare();
        let toolchain = Toolchain::newest(&ProjectRoot::new(repository().to_path_buf()))
            .await
            .expect("the repository pins a Rust toolchain by version");

        let pins = project.directory.path().join("mise.toml");
        std::fs::write(&pins, format!("[tools]\nrust = \"{}\"\n", toolchain.get()))
            .expect("the test writes the mise.toml of the project");
        trust(&pins);
        project.write("Cargo.toml", PACKAGE);
        project.write("src/lib.rs", PASSING);
    ...
    async fn run(&self) -> Outcome {
        CheckMinimalDeps.run(&self.context(), &()).await
    }
...
// checkminimaldeps[verify args.none]
#[test]
fn action_declares_no_argument() {
    let schema = <CheckMinimalDeps as Action>::Args::schema();

    assert!(schema.arguments().is_empty());
}

// checkminimaldeps[verify name]
#[test]
fn action_identifies_itself_as_check_minimal_deps() {
    let name = CheckMinimalDeps.name();

    assert_eq!(name.get(), "check-minimal-deps");
}

// checkminimaldeps[verify tests.passed]
// checkminimaldeps[verify tool.cargo]
#[tokio::test]
...
// checkminimaldeps[verify copy.unavailable]
#[tokio::test]
...
// The library reports the toolchain that built it, and the test in it fails
// when that toolchain is the nightly of the update. The run passes, so the
// tests built on the toolchain that the project pins.
// checkminimaldeps[verify tests.toolchain]
#[tokio::test]
async fn run_builds_the_tests_on_the_toolchain_of_the_project() {
    let project = Project::with_library(REPORTING_TOOLCHAIN);
...
// checkminimaldeps[verify copy.disposable]
#[tokio::test]
...
// checkminimaldeps[verify tests.error]
#[tokio::test]
...
// checkminimaldeps[verify tests.failed]
#[tokio::test]
...
// checkminimaldeps[verify update.failed]
#[tokio::test]
...
// checkminimaldeps[verify copy.paths]
#[tokio::test]
...
// checkminimaldeps[verify skip.links]
#[cfg(unix)]
#[tokio::test]
...
// checkminimaldeps[verify skip.git]
#[tokio::test]
...
// checkminimaldeps[verify skip.target]
#[tokio::test]
...
// what that lockfile holds. Cargo refuses the option of the update on every
// channel but the nightly, so an update that reached this far ran there.
// checkminimaldeps[verify tests.locked]
// checkminimaldeps[verify tool.toolchain]
// checkminimaldeps[verify update.operation]
#[tokio::test]
...
// checkminimaldeps[verify roots.all]
#[tokio::test]
...
// checkminimaldeps[verify tool.missing]
#[tokio::test]
...
// checkminimaldeps[verify skip.missing]
#[tokio::test]
...
// The project pins a cargo, so the run gets past the tool and stops over the
// channel that the update needs. The message names the nightly, which tells
// the two conditions apart.
// checkminimaldeps[verify tool.unpinned]
#[tokio::test]
async fn run_without_a_nightly_toolchain_stops() {
    let project = Project::without_nightly().await;
    ...
    assert!(
        source.to_string().contains("nightly"),
        "expected the error to name the nightly, got {source}"
    );
}
