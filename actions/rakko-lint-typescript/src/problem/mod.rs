...
/// Returns the path without the prefix that names where oxlint started
///
/// A run names the current directory as the place to look, so oxlint reports a
/// path that is already relative to the project root. A path that names the
/// current directory in front of it loses that part, because a reader and a
/// code host expect the plain path.
///
